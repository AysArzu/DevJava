...
public class Lambda08 {
    public static void main(String[] args) {
        ...
        yedidenBuyukEnKucukCiftSayi(nums);
    ...
    public static void yedidenBuyukEnKucukCiftSayi(List<Integer> nums) {
        //Integer sayi = nums.stream().filter(t -> t > 7 && t % 2 == 0).sorted().reduce((t, u) -> t).get();
        Integer sayi = nums.stream().distinct().filter(t -> t > 7 && t % 2 == 0).sorted().findFirst().get();
