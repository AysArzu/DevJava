...
/*        for (String w : r){ //4 defa calisacak
        ...
        System.out.println(r); //HATA  for each te silme yapamiyorsun!!!!*/
        //[Manisa, Nigde, Tokat, Van]

        for (int i = 0; i < r.size(); i++) {
            if (r.get(i).contains("a")) {
                r.remove(i);
                i--;// silince yeniden indexleme yapar ilk index sifir iken burada -1 olur yukarida i++ ile index yine sifir olur
            }
        }
